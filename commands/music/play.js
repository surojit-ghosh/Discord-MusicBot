...
export default {
    ...
    aliases: ['p'],
    description: 'Play music from music name or url or playlist url',
    run: async (client, message, args) => {
        ...
        if (!search) return message.reply({
            embeds: [{
                color: client.color.error,
                description: `Usage - \`${client.prefix}play <song | playlist>\``
            }]
        });
        let node = client.manager.nodes.get(client.config.lavalink.id);
